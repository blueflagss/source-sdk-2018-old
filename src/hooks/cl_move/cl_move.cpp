...
void __vectorcall hooks::cl_move::hook( float accumulated_extra_samples, bool final_tick ) {
    if ( !g_interfaces.client_state || !g_interfaces.engine_client->is_connected( ) || !g_interfaces.engine_client->is_in_game( ) )
        return reinterpret_cast< decltype( &cl_move::hook ) >( cl_move::original.trampoline( ).address( ) )( accumulated_extra_samples, final_tick );
    ...
    reinterpret_cast< decltype( &cl_move::hook ) >( cl_move::original.trampoline( ).address( ) )( accumulated_extra_samples, final_tick );
