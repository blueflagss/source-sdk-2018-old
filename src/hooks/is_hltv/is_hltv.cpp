...
bool __fastcall hooks::is_hltv::hook( REGISTERS ) {
    static auto accumulate_layers_call = signature::find( _xs( "client.dll" ), _xs( "84 C0 75 0D F6 87" ) ).get< void * >( );
    
    if ( _ReturnAddress( ) == accumulate_layers_call )
        return true;

    static auto setup_velocity_call = signature::find( _xs( "client.dll" ), _xs( "84 C0 75 38 8B 0D ? ? ? ? 8B 01 8B 80" ) ).get< void * >( );
