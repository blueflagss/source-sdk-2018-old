...
void __fastcall hooks::calc_view::hook( REGISTERS, vector_3d &eye_origin, vector_3d &eye_angles, float &z_near, float &z_far, float &fov ) {
    ...
    if ( !entity || entity != globals::local_player )
...
void hooks::calc_view::init( ) {
    original = safetyhook::create_inline( signature::find( "client.dll", _xs( "55 8B EC 53 8B 5D 08 56 57 FF 75 18 8B F1" ) ).get< void * >( ),
                                          calc_view::hook );
}