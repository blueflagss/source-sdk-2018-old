...
RayTracer::Ray::Ray( const vector_3d &startPoint, const vector_3d &endPoint ) : m_startPoint( startPoint ) {
...
RayTracer::Hitbox::Hitbox( const vector_3d &mins, const vector_3d &maxs, const float radius ) : m_mins( mins ), m_maxs( maxs ), m_radius( radius ) {}

RayTracer::Hitbox::Hitbox( const std::tuple< vector_3d, vector_3d, float > &initTuple ) : m_mins( std::get< 0 >( initTuple ) ), m_maxs( std::get< 1 >( initTuple ) ), m_radius( std::get< 2 >( initTuple ) ) {}
...
void RayTracer::TraceFromCenter( const Ray &ray, const Hitbox &hitbox, Trace &trace, int flags ) {
    ...
    auto dot = glm::dot( unitHitbox, unitDesired );
    auto cross = glm::cross( unitHitbox, unitDesired );

    vector_3d rotatedDirection;
    ...
    if ( a == 0 ) {
        ...
        if ( rotatedDirection.x > 0 ) {
            ...
            auto newLength = glm::length( maxsOffset ) + hitbox.m_radius;
            auto offset = ( maxsOffset * ( newLength ) / glm::length( maxsOffset ) );
...
void RayTracer::TraceHitbox( const Ray &ray, const Hitbox &hitbox, Trace &trace, int flags ) {
    ...
    auto minsStart = vector_3d( rotatedStart.x - minsAdjusted, rotatedStart.y, rotatedStart.z );
    auto maxsStart = vector_3d( rotatedStart.x - maxsAdjusted, rotatedStart.y, rotatedStart.z );

    auto b_c = 2.f *
               ( rotatedStart.y * rotatedDirection.y +
                 rotatedStart.z * rotatedDirection.z );

    auto b_smins = 2.f * ( glm::dot( minsStart, rotatedDirection ) );
    auto b_smaxs = 2.f * ( glm::dot( maxsStart, rotatedDirection ) );

    auto c_c = rotatedStart.y * rotatedStart.y +
               rotatedStart.z * rotatedStart.z -
               hitbox.m_radius * hitbox.m_radius;

    auto c_smins = glm::dot( minsStart, minsStart ) -
                   hitbox.m_radius * hitbox.m_radius;
    auto c_smaxs = glm::dot( maxsStart, maxsStart ) -
                   hitbox.m_radius * hitbox.m_radius;
