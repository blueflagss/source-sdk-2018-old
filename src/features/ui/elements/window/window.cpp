...
void penumbra::window::paint( ) {
    ...
    auto title_text_dimensions = render::get_text_size( fonts::montserrat_semibold, this->game_name );

    if ( !this->game_name.empty( ) )
        render::string( fonts::montserrat_semibold, this->position.x - 10 + this->size.x - title_text_dimensions.x, this->position.y + ( this->is_using_font ? 9.5f : 6.5f ), color{ 180, 180, 180, 80 * globals::fade_opacity[ this->get_main_window( ) ] }, this->game_name );
    ...
    if ( !this->tab_objects.empty( ) ) {
        ...
        render::scissor_rect( this->position, this->size, [ & ] {
            animations::lerp_to( HASH( this->name.c_str( ) ), this->current_page * ( this->size.x / this->tab_objects.size( ) ), 0.1f );

            render::gradient_rect( this->position.x, this->position.y + this->titlebar_size.y, this->titlebar_size.x, this->titlebar_size.y / 2, color{ 12, 12, 12, 30 * globals::fade_opacity[ this->get_main_window( ) ] }, color{ 0, 0, 0, 0 }, true );
        } );

        render::gradient_rect( this->position.x, this->position.y + this->titlebar_size.y + this->tab_height - 2, this->titlebar_size.x, this->titlebar_size.y / 2, color{ 12, 12, 12, 80 * globals::fade_opacity[ this->get_main_window( ) ] }, color{ 0, 0, 0, 0 }, true );
    ...
    if ( this->is_window_resizeable ) {
        ...
        auto resize_indicator_position = glm::vec2( this->position + this->size - glm::vec2( 7.0f, 5.0f ) - icon_dimensions );
        ...
        render::string( fonts::font_awesome, resize_indicator_position, color{ 100, 100, 100, 255 * globals::fade_opacity[ this->get_main_window( ) ] }.lerp( color{ 255, 255, 255, 255 * globals::fade_opacity[ this->get_main_window( ) ] }, animation.value ), ICON_FA_EXPAND_ALT, true );
    }
    ...
    render::rect( this->position.x - 2, this->position.y - 2, this->size.x + 4, this->size.y + 4, color{ 9, 9, 9, 50 * globals::fade_opacity[ this->get_main_window( ) ] }, 4.0f );
    render::rect( this->position.x - 1, this->position.y - 1, this->size.x + 2, this->size.y + 2, color{ 100, 100, 100, 100 * globals::fade_opacity[ this->get_main_window( ) ] }, 4.0f );
    ...
    if ( !this->tab_objects.empty( ) ) {
        ...
        for ( int i = { }; i < this->tab_objects.size( ); i++ ) {
            ...
            object->position = glm::vec2(
                    std::roundf( this->position.x + 2 + i * ( this->size.x / this->tab_objects.size( ) ) - 2 ),
...
void penumbra::window::handle_object( const std::shared_ptr< penumbra::object > &object ) {
    switch ( object->element_type ) {
        ...
        case TYPE_LABEL: {
            this->cursor_position.y += 22.5f;
        } break;
        case TYPE_BUTTON: {
            object->size = glm::vec2{ this->size.x - 21.0f, 25.0f };

            this->cursor_position.y += object->size.y + 7.0f;
        } break;
        case TYPE_COMBOBOX: {
            object->size = glm::vec2{ this->size.x - 24.0f, 25.0f };
        ...
        case TYPE_INDICATOR: {
            object->size = glm::vec2{ this->size.x - 24.0f, 5.0f };
