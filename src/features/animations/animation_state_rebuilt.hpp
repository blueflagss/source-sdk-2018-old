...
namespace valve_math
{
    ...
    __forceinline float remap_val_clamped( float val, float A, float B, float C, float D ) {
        if ( A == B )
            return val >= B ? D : C;
    ...
    __forceinline float angle_mod( float a ) {
    ...
    __forceinline void angle_vectors( const vector_3d &angles, vector_3d *forward, vector_3d *right, vector_3d *up ) {
        float sr, sp, sy, cr, cp, cy;

        math::sin_cos( math::deg_to_rad( angles.y ), &sy, &cy );
        math::sin_cos( math::deg_to_rad( angles.x ), &sp, &cp );
    ...
    __forceinline float clamp_cycle( float cycle_in ) {
...
public:
    ...
    void update_animation_state( c_csgo_player_animstate *state, vector_3d &angles, bool force_update );
    void increment_layer_weight( c_csgo_player_animstate *state, int layer_idx );
    void setup_whole_body_action( c_csgo_player_animstate *state );
    void setup_velocity( c_csgo_player_animstate *state );
    void update_anim_layer( c_csgo_player_animstate *state, int layer_idx, int seq, float playback_rate, float weight, float cycle );
    void trigger_animation_events( c_csgo_player_animstate *state );
    void setup_movement( c_csgo_player_animstate *state );
    void setup_alive_loop( c_csgo_player_animstate *state );
    void setup_flashed_reaction( c_csgo_player_animstate *state );
    void setup_flinch( c_csgo_player_animstate *state );
    void setup_aim_matrix( c_csgo_player_animstate *state );
    void setup_weapon_action( c_csgo_player_animstate *state );
    void setup_lean( c_csgo_player_animstate *state );
...
inline c_animation_state_rebuilt g_rebuilt = { };