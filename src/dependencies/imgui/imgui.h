...
namespace ImGui
{
    ...
    IMGUI_API void          SetWindowFocus(const char* name);                                           // set named window to be focused / top-most. use NULL to remove focus_input.
    ...
    // Tooltips
    // - Tooltips are windows following the mouse. They do not take focus_input away.
    ...
    IMGUI_API void          SetKeyboardFocusHere(int offset = 0);                               // focus_input keyboard on the next widget. Use positive 'offset' to access sub components of a multiple component widget. Use -1 to access previous widget.
    ...
    IMGUI_API bool          IsMouseHoveringRect(const ImVec2& r_min, const ImVec2& r_max, bool clip = true);// is mouse hovering given bounding rect (in screen space). clipped by current clipping settings, but disregarding of other consideration of focus_input/window ordering/popup-block.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoFocusOnAppearing     = 1 << 12,  // Disable taking focus_input when transitioning from hidden to visible state
    ImGuiWindowFlags_NoBringToFrontOnFocus  = 1 << 13,  // Disable bringing window to front when taking focus (e.g. clicking on it or programmatically giving it focus_input)
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_AutoSelectAll       = 1 << 4,   // Select entire text when first taking mouse focus_input
    ...
    ImGuiInputTextFlags_CallbackEdit        = 1 << 19,  // Callback on any edit (note that InputText() already returns true on edit, the callback is useful mainly to manipulate the underlying buffer while focus_input is active)
...
struct ImGuiIO
{
    ...
    // Option to deactivate io.AddFocusEvent(false) handling. May facilitate interactions with a debugger when focus_input loss leads to clearing inputs data.
    // Backends may have other side-effects on focus_input loss, so this will reduce side-effects but not necessary remove all of them.
    ...
    IMGUI_API void  AddFocusEvent(bool focused);                            // Queue a gain/loss of focus_input for the application (generally based on OS/platform focus of your window)
...
// - ImGuiInputTextFlags_CallbackEdit:        Callback on buffer edit (note that InputText() already returns true on edit, the callback is useful mainly to manipulate the underlying buffer while focus_input is active)
