...
// Flags
typedef int ImGuiActivateFlags;         // -> enum ImGuiActivateFlags_      // Flags: for navigation/focus_input function (will be for ActivateItem() later)
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_NoNavDefaultFocus        = 1 << 4,  // false     // Disable item being a candidate for default focus_input (e.g. used by title bar items)
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_NoNavFocus             = 1 << 18,  // don't override navigation focus_input when activated (FIXME: this is essentially used everytime an item uses ImGuiItemFlags_NoNav, but because legacy specs don't requires LastItemData to be set ButtonBehavior(), we can't poll g.LastItemData.InFlags)
    ImGuiButtonFlags_NoHoveredOnFocus       = 1 << 19,  // don't report as hovered when nav focus_input is on this item
...
enum ImGuiSelectableFlagsPrivate_
{
    ...
    ImGuiSelectableFlags_SelectOnNav            = 1 << 21,  // (WIP) Auto-select when moved into. This is not exposed in public API as to handle multi-select and modifiers we will need user to explicitly control focus_input scope. May be replaced with a BeginSelection() API.
...
enum ImGuiFocusRequestFlags_
{
    ...
    ImGuiFocusRequestFlags_RestoreFocusedChild  = 1 << 0,   // Find last focused child (if any) and focus_input it instead.
    ImGuiFocusRequestFlags_UnlessBelowModal     = 1 << 1,   // Do not set focus_input if the window is below a modal.
};
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImVector<char>          InitialTextA;           // backup of end-user buffer at the time of focus_input (in UTF-8, unaltered)
...
enum ImGuiInputFlags_
{
    ...
    //   will register a route and only succeed when parent window is in the focus_input stack and if no-one
    ...
    ImGuiInputFlags_RouteFocused        = 1 << 8,   // (Default) Register focused route: Accept inputs if window is in focus_input stack. Deep-most focused window takes inputs. ActiveId takes inputs over deep-most focused window.
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_FocusApi              = 1 << 9,   // Requests from focus_input API can land/focus/activate items even if they are marked with _NoTabStop (see NavProcessItemForTabbingRequest() for details)
...
struct ImGuiNavItemData
{
    ...
    ImGuiID             FocusScopeId;   // Init,Move    // Best candidate focus_input scope ID
...
struct ImGuiContext
{
    ...
    ImVector<ImGuiWindow*>  WindowsFocusOrder;                  // Root windows, sorted in focus_input order, back to front.
    ...
    ImGuiWindow*            MovingWindow;                       // Track the window we clicked on (in order to preserve focus_input). The actual window that is moved is generally MovingWindow->RootWindow.
    ...
    ImGuiID                 NavJustMovedToFocusScopeId;         // Just navigated to this focus_input scope id (result of a successfully MoveRequest).
...
//#endif
    ...
    //   So in order to identify a set multiple lists in same window may each need a focus_input scope.
    //   If you imagine an hypothetical BeginSelectionGroup()/EndSelectionGroup() api, it would likely call PushFocusScope()/EndFocusScope()
    // - Shortcut routing also use focus_input scope as a default location identifier if an owner is not provided.
    ...
    // Refactored focus_input/nav/tabbing system in 1.82 and 1.84. If you have old/custom copy-and-pasted widgets that used FocusableItemRegister():
