...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // Release focus_input when we click outside
    ...
    if (is_multiline)
    {
        // For focus_input requests to work on our multiline we need to ensure our child ItemAdd() call specifies the ImGuiItemFlags_Inputable (ref issue #4761)...
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    //   - (1) it would require focus_input scope to be set, need exposing PushFocusScope() or equivalent (e.g. BeginSelection() calling PushFocusScope())
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
        ...
        {
            // To do so we claim focus_input back, restore NavId and then process the movement request for yet another frame.
...
void ImGui::EndMainMenuBar()
{
    ...
    // When the user has left the menu layer (typically: closed menus through activation of an item), we restore focus_input to the previous window
    // FIXME: With this strategy we won't be able to restore a NULL focus_input.
    ImGuiContext& g = *GImGui;
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // Sub-menus are ChildWindow so that mouse can be hovering across them (otherwise top-most popup menu would steal focus_input and not allow hovering on parent menu)
